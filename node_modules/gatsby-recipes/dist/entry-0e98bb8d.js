...
const makePath = (root, relativePath) => path__default['default'].join(root, relativePath);
...
function isDefaultExport(node) {
  if (!node || !t$1.isMemberExpression(node)) {
    return false;
  }
  ...
  if (!t$1.isIdentifier(object) || object.name !== `module`) return false;
  if (!t$1.isIdentifier(property) || property.name !== `exports`) return false;
  return true;
}
...
const isLiteral = node => t$1.isLiteral(node) || t$1.isStringLiteral(node) || t$1.isNumericLiteral(node);

const getValueFromNode = node => {
  if (t$1.isTemplateLiteral(node)) {
...
const getObjectFromNode = nodeValue => {
  ...
  const props = nodeValue.properties.reduce((acc, curr) => {
    ...
    } else if (t$1.isObjectExpression(curr.value)) {
...
const getOptionsForPlugin = node => {
  if (!t$1.isObjectExpression(node) && !t$1.isLogicalExpression(node)) {
    return undefined;
  }

  let options; // When a plugin is added conditionally with && {}

  if (t$1.isLogicalExpression(node)) {
...
const getNameForPlugin = node => {
  if (t$1.isStringLiteral(node) || t$1.isTemplateLiteral(node)) {
      ...
      return {
        visitor: {
          ExpressionStatement(path) {
            ...
            if (shouldAdd) {
              if (t$1.isCallExpression(pluginNodes.value)) {
      ...
      return {
        visitor: {
          ExpressionStatement: path => {
            ...
            if (t$1.isCallExpression(plugins.value)) {
        ...
        visitor: {
          ExpressionStatement(path) {
            ...
            if (siteMetadataExists) {
              ...
              right.properties.unshift(t$1.objectProperty(t$1.identifier(`siteMetadata`), newSiteMetadata));
            }
...
const makePath$2 = root => path__default['default'].join(root, `.gitignore`);
...
exports.visit = visit;
//# sourceMappingURL=entry-0e98bb8d.js.map
