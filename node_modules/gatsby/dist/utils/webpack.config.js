...
var _staticQueryMapper = require("./webpack/static-query-mapper");

var _forceCssHmrForEdgeCases = require("./webpack/force-css-hmr-for-edge-cases");
...
const {
  CacheFolderResolver
} = require(`./webpack/cache-folder-resolver`);
...
module.exports = async (program, directory, suppliedStage, port, {
  ...
  function getEntry() {
    switch (stage) {
      ...
      case `develop-html`:
        return {
          main: process.env.GATSBY_EXPERIMENTAL_DEV_SSR ? directoryPath(`.cache/ssr-develop-static-entry`) : directoryPath(`.cache/develop-static-entry`)
        };
  ...
  function getPlugins() {
    ...
    }), plugins.virtualModules(), new BabelConfigItemsCacheInvalidatorPlugin()];
  ...
  if (process.env.GATSBY_EXPERIMENTAL_PRESERVE_WEBPACK_CACHE && (stage === `build-javascript` || stage === `build-html`)) {
    ...
    const cacheConfig = {
      ...
      cacheLocation,
      buildDependencies: {
        config: [__filename, ...store.getState().flattenedPlugins.filter(plugin => plugin.nodeAPIs.includes(`onCreateWebpackConfig`)).map(plugin => path.join(plugin.resolve, `gatsby-node.js`))]
      }
    };
  ...
  if (fastRefreshPlugin) {
    ...
    for (const rule of getConfig().module.rules) {
      var _rule$use$loader, _rule$use;

      if (!rule.use && !rule.loader) {
        continue;
      }

      const ruleLoaders = Array.isArray(rule.use) ? rule.use.map(useEntry => typeof useEntry === `string` ? useEntry : useEntry.loader) : [(_rule$use$loader = (_rule$use = rule.use) === null || _rule$use === void 0 ? void 0 : _rule$use.loader) !== null && _rule$use$loader !== void 0 ? _rule$use$loader : rule.loader];
