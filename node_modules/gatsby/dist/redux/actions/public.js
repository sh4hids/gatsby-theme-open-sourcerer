...
actions.deleteNode = (node, plugin) => {
  let id; // TODO(v4): Remove this deprecation warning and only allow deleteNode(node)

  if (node && node.node) {
    let msg = `Calling "deleteNode" with {node} is deprecated. Please pass ` + `the node directly to the function: deleteNode(node)`;
...
actions.touchNode = (node, plugin) => {
  ...
  if (node && node.nodeId) {
    let msg = `Calling "touchNode" with an object containing the nodeId is deprecated. Please pass ` + `the node directly to the function: touchNode(node)`;
  ...
  const nodeId = (_node = node) === null || _node === void 0 ? void 0 : _node.id;
