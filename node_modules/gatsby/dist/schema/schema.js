...
const rebuildSchemaWithSitePage = async ({
  ...
  (0, _derivedTypes.deleteFieldsOfDerivedTypes)({
    typeComposer
  });
...
const createTypeComposerFromGatsbyType = ({
  ...
  switch (type.kind) {
    case _typeBuilders.GatsbyGraphQLTypeKind.OBJECT:
      {
        typeComposer = ObjectTypeComposer.createTemp({ ...type.config,
    ...
    case _typeBuilders.GatsbyGraphQLTypeKind.INPUT_OBJECT:
      {
        typeComposer = InputTypeComposer.createTemp({ ...type.config,
    ...
    case _typeBuilders.GatsbyGraphQLTypeKind.UNION:
      {
        typeComposer = UnionTypeComposer.createTemp({ ...type.config,
          types: () => {
    ...
    case _typeBuilders.GatsbyGraphQLTypeKind.INTERFACE:
      {
        typeComposer = InterfaceTypeComposer.createTemp({ ...type.config,
          fields: () => schemaComposer.typeMapper.convertOutputFieldConfigMap(type.config.fields),
          interfaces: () => {
            if (type.config.interfaces) {
              return type.config.interfaces.map(iface => {
  ...
  thirdPartySchemas.forEach(schema => {
    ...
    Object.keys(types).forEach(typeName => {
      ...
      if (type !== schemaQueryType && !isSpecifiedScalarType(type) && !isIntrospectionType(type) && type.name !== `Date` && type.name !== `JSON`) {
        const typeHasFields = type instanceof GraphQLObjectType || type instanceof GraphQLInterfaceType; // Workaround for an edge case typical for Relay Classic-compatible schemas.
  ...
  schemaComposer.forEach(typeComposer => {
    if (typeComposer instanceof ObjectTypeComposer || typeComposer instanceof InterfaceTypeComposer) {
      typeComposer.getFieldNames().forEach(fieldName => {
        ...
        typeComposer.extendField(fieldName, {
          resolve: field.resolve && field.resolve !== defaultResolver ? wrappingResolver(field.resolve) : defaultResolver
        });
      });
    }
  });
...
const addInferredChildOfExtension = ({
  ...
  const shouldInfer = typeComposer.getExtension(`infer`); // With `@dontInfer`, only parent-child
  // relations explicitly set with the `@childOf` extension are added.

  if (shouldInfer === false) return;
  const parentTypeName = typeComposer.getTypeName(); // This is expensive.
...
const addTypeToRootQuery = ({
  ...
  const sortInputTC = (0, _sort.getSortInput)({
    schemaComposer,
    typeComposer
  });
...
const isNodeInterface = interfaceTypeComposer => interfaceTypeComposer.hasExtension(`nodeInterface`) || interfaceTypeComposer.hasInterface(`Node`);
